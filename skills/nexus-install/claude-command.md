...
```bash
/home/luanh/.nexus/scripts/nexus install "/path/to/source" --skill "skill-a" --skill "skill-b"
```

Replace the example values with the concrete user-provided source and skill names, passing the source and each repeated `--skill` value as separate argv entries. Never use `eval`. Report the command output.
