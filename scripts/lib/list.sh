help_text() {
  cat <<'EOF'
Usage: nexus <bootstrap|setup|link|install|update|remove|new|list|check|global|help>
...
check        Ask GitHub which installed skills are behind: check [--json]; the last result: check --last [--json].
